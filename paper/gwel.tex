...
Traffic:\\ Free Baselines, Corpus Ceilings and Graded Actions in Sub-Billion
VLMs}\vspace{-0.5em}}
...
decision by reinforcement learning on 7B-class models and read it from the
output distribution after generation. We ask what a 500M model can supply
before it writes a token, and price every candidate against the baseline this
literature omits: randomising between fixed configurations, which needs no
signal and traces a convex hull any adaptive policy must clear.

The answer inverts between two regimes. Across a four-dataset mixture, output
entropy never clears the hull, because reading it requires completing the pass
it prices; a probe on pre-generation activations clears it, but so do the
image's own pixel dimensions, read from the file header for nothing and within
$0.010$ of the probe at every operator preference, and a policy given only the
dataset label dominates both. Inside a workload the ranking reverses: image
size falls to $0.508$ AUROC and clears nothing on either of the two workloads
measured, while output entropy on a graded resolution ladder clears by $+0.041$
to $+0.001$ on the one where the model is strong and at one preference of four
on the one where it is not, and binary escalation never clears at all. \textbf{Which signal is worth reading is a property of the traffic,
and a method evaluated only on a mixture will recommend the wrong one for a
workload.} The probe's apparent advantage ($0.760$ against $0.617$) is a
between-domain effect, and it is one of eight claims lost to family-wise
correction over the sixty-three paired comparisons reported here.

What survives is graded action and measured cost. On $1200$ document pages,
$93\%$ of what escalation repairs is repaired below the top rung, and accuracy
saturates at a pixel resolution shared by four models spanning $8.6\times$
parameters, two lineages and three tokenisers, and by a second corpus that
reaches the same target while spending half the visual tokens to get there. A fifth model, whose tile grid
moves independently of its input, makes that ceiling an upper bound rather than
a constant and separates the quantities the others confound: one pixel rung at
a fixed token budget buys $+0.108$, while tripling the token budget at fixed
pixels buys $+0.024\ [-0.006, +0.054]$. We state the procedure that locates the
ceiling and the sample size it needs.
\end{abstract}
...
\begin{enumerate}\itemsep2pt
  ...
  stays within $0.010$ of a probe on activations, while output entropy never
  clears it; inside one workload the ranking inverts, image size falls to
  $0.508$ AUROC and clears nothing, and only entropy on a graded action space
  clears the hull ($+0.041$ to $+0.001$). A policy that randomises with the
  probe's mid-prefill abort clears nothing either, which places the probe's
  margin in its signal and not in its read cost. A method evaluated only on a
  mixture will recommend the wrong signal for a workload
  ...
  tokens, and by a second corpus that reaches it while spending half the visual
  tokens, so a token budget cannot be what runs out. The ceiling is measurable
  once by a procedure we state and price at about $300$ pages. A fifth model,
  whose token budget stays flat as resolution rises, saturates a rung earlier,
  which makes the corpus ceiling an upper bound rather than a constant
...
either costing}, and every one of those clearances survives family-wise
correction. Paired against the probe over the same resamples, the free signal
differs by at most $0.010$ either way, and of the preferences where one of them
nominally wins, none survives correction. On this mixture the pre-generation probe
...
single workload that between-domain axis is gone: the free descriptor collapses
to chance, and what clears the hull is a signal read from the model, spent on a
graded action space rather than on a binary one. Neither regime is the general
case, and a method evaluated only in the first will recommend the wrong thing
for the second.

The two halves rest on different amounts of evidence and we separate them here.
The negative half, that a free descriptor stops working once the between-domain
axis is removed, is measured on both workloads we collected. The positive half,
that a model-read signal on a graded action space clears the hull, is measured
on DocVQA at three preferences of four and on InfographicVQA at one of four
(\S\ref{sec:ladder}); on the latter the serving model answers only $0.368$ of
the corpus at its ceiling, so most queries are unanswerable at any rung and
there is little for any policy to allocate. What we claim generally is the
inversion of the \emph{ordering} between the two signals. How much the winning
side is worth is corpus-dependent, and on a corpus where the model is weak it
is worth little.
...
\paragraph{The procedure on a corpus it has not seen, and a prediction it
refutes.} An earlier version of this paper predicted that a benchmark of denser
type should place the ceiling higher than DocVQA's $1152$\,px. We collected one
and ran Algorithm~3 on it unchanged: $500$ InfographicVQA pages, the same task,
the same ANLS metric, the same pixel targets, and pages carrying a far wider
range of type sizes (Table~\ref{tab:corpora}).
...
\begin{tabular}{lccccc}
\toprule
& $384$ & $768$ & $1152$ & $2048$ & ceiling \\
\midrule
DocVQA ($n{=}1200$) & $0.279$ & $0.662$ & $\mathbf{0.748}$ & $0.746$ & $1152$ \\
InfographicVQA ($n{=}500$) & $0.230$ & $0.282$ & $\mathbf{0.368}$ & $0.360$ & $1152$ \\
\midrule
\multicolumn{6}{l}{\emph{median visual tokens spent at each target}} \\
DocVQA & $64$ & $320$ & $\mathbf{640}$ & $1088$ & \\
InfographicVQA & $64$ & $192$ & $\mathbf{320}$ & $576$ & \\
\bottomrule
\end{tabular}}
\caption{The same procedure on two corpora. The top steps are
$-0.002\ [-0.024, +0.022]$ and $-0.008\ [-0.044, +0.028]$, both inside the
precision bar of Algorithm~3's step~6. The $768 \to 1152$ step pays on both,
$+0.086$ each.}
\label{tab:corpora}
\end{table}

\textbf{The prediction fails and the claim it was attached to survives in a
stronger form.} The ceiling does not move: both corpora stop paying at
$1152$\,px, and on both the top step is a measured null rather than an absence
of measurement. What moves is the price. The same pixel target costs $640$
visual tokens on DocVQA and $320$ on InfographicVQA, whose pages are tall and
narrow and fill fewer patches at a given longest edge, so two corpora reach an
identical ceiling while spending a factor of exactly two apart. Read alongside
the cross-model result above, the ceiling is invariant to the serving model at
fixed corpus and to the corpus at fixed pixel target, while the token count at
that ceiling varies by $2.05\times$ across models and $2.00\times$ across
corpora. \textbf{A token budget cannot be what runs out if two corpora exhaust
it at half each other's spend.}

We report the failed prediction rather than deleting it. It was the reason to
collect this corpus, and its failure upgrades ``$1152$\,px is DocVQA's number''
to ``$1152$\,px is where two document corpora stop paying''. That is wider than
we were entitled to before and narrower than universality: both are
document-QA, and a corpus of genuinely finer print remains untested.

One thing does not transfer with it. On InfographicVQA the model is far weaker
($0.368$ against $0.748$ at the ceiling), and the ladder's advantage over the
hull nearly disappears: it clears at one operator preference of four
($+0.006\ [+0.002, +0.011]$ at $V{=}800$) rather than at three. The free image
descriptor fails on this workload as on the other, at every preference, which
is a second confirmation of the inversion in \S\ref{sec:policy}. Where a model
is weak enough that most queries are unanswerable at any rung, there is little
for any per-query policy to allocate.
...
the free descriptor fails on both workloads measured; the graded ladder clears
on the one where the model is strong, marginally on the other \\
\addlinespace
...
measured on two document corpora and five models; untested outside document QA \\
\addlinespace
...
measured on DocVQA; one preference of four on InfographicVQA; null where no top
rung exists \\
\addlinespace
...
than a constant. It was measured on two corpora, DocVQA and InfographicVQA,
which stop paying at the same pixel target while spending a factor of two apart
in visual tokens to reach it. Both are document-QA, so the claim we are
entitled to is that document pages of this kind have a legibility ceiling near
$1152$\,px that bounds every model's useful resolution from above and that
Algorithm~3 locates in about $300$ pages. We predicted that the denser of the
two would place the ceiling higher and it did not, which is reported in
\S\ref{sec:ladder} rather than removed. A corpus of genuinely finer print, or
of a different medium, remains untested, and the procedure rather than the
number is what we claim transfers. The mechanism behind the ceiling is settled only on the model that lets both
...
corrected rather than nominal. Sixty-three paired comparisons at the nominal
$5\%$ level carry a $96\%$ chance of at least one false positive, so we convert
each bootstrap to a two-sided p-value and run Holm's step-down over the whole
family. Every comparison carries its own resample vector, so all sixty-three
are scored by the same bootstrap rather than by an approximation to a reported
interval.

Forty-seven of the sixty-three clear the nominal level and thirty-nine survive
the step-down. Eight claims are lost, and they are worth naming individually
because four of them are convenient for our thesis. The probe's AUROC advantage
...
tile-budget control adjust to $0.745$ and $0.722$. And the second corpus's
weakest rung gain, $+0.052$ from $384$ to $768$\,px on InfographicVQA, adjusts
to $0.331$; the step that sets that corpus's ceiling, $+0.086$ from $768$ to
$1152$\,px, survives, so the ceiling determination does not depend on the lost
one. Every cost comparison survives, as do the pixel-axis gains, the
...
any stops holding; it currently checks sixty-eight, including the negative
...
workload the same measurement inverts: the free descriptor falls to chance and
only a model-read signal on a graded action space clears the no-signal
baseline. Neither regime is the general case, which is the result we would put
first.

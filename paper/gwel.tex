...
resolution when the model appears uncertain. Published escalation policies learn
this decision by reinforcement learning on 7B-class models and read it from the
output distribution after generation. We ask what a 500M-parameter model already
knows before it writes a token, and the answer did not survive our own audits
intact.

A linear probe on pre-generation activations appears to predict escalation
value better than output entropy ($0.760$ against $0.617$). Audited, most of
that advantage is a domain effect: free image size reaches $0.748$ on the same
mixture; inside a single dataset the probe plateaus at $0.572$ against entropy's
$0.663$ given nine times the training data, at every depth, for four probe
families; and where the two signals disagree, the disagreement predicts the
source dataset at $0.738$. The headline ranking gain is nominally significant
and does not survive correction across the fourteen paired comparisons this
paper reports.

What survives is a decision question and a cost question. Randomising between
fixed configurations traces a convex hull that any adaptive policy must clear.
The standard entropy threshold never clears it on our mixture, because its read
requires the pass it prices. A cost-derived rule (escalate where the calibrated
expected gain exceeds the measured marginal latency over the operator's stated
value of a correct answer) clears the hull by up to $+0.054$ when its signal is
read mid-prefill, and, extended to a resolution ladder, by $+0.006$ to $+0.042$
on homogeneous document traffic. On $1200$ document pages, $93\%$ of what
escalation repairs is repaired below the top rung, and accuracy saturates at a
pixel resolution that four models agree on, spanning $8.6\times$ parameters,
two lineages and three ways of turning pixels into tokens: a property of the
corpus, measurable once. A fifth model, whose token budget does not grow with
resolution, stops a rung earlier: the corpus ceiling is an upper bound on what
a model needs rather than a constant, and it bounds how much of that ceiling is
the pixels and how much the sequence length.

Sixty-three claims, including every negative result and the limits of the
positive ones, are re-derived from the run records by a harness that fails
loudly when any stops holding, with paired comparisons corrected for
family-wise error.
\end{abstract}
...
Fig.~\ref{fig:method} is where we end up: a single cheap pass, a score, and a
...
\begin{enumerate}\itemsep2pt
  ...
  per example reveals the probe escalating $1.19\text{--}1.33\times$ larger
  images than entropy; following that thread, free image size scores $0.748$
  against the probe's $0.761$, and inside a single dataset the probe reaches
  $0.572$ where output entropy, fitted on nothing, reaches $0.663$, at any depth
  and with any of four probe families. The probe's ranking advantage is a
  between-domain effect, and it does not survive family-wise correction
  (\S\ref{sec:confound}, \S\ref{sec:singledomain}).
  \item \textbf{A no-signal baseline that most routing fails.} Randomising
  between fixed configurations reaches every point on their convex hull with no
  signal at all, and this is the comparison the escalation literature does not
  make. The standard entropy threshold never clears that hull on our mixture,
  because its read requires the pass it prices; the probe clears it by
  $+0.008$ to $+0.054$, an advantage owed to its mid-prefill abort rather than
  to its ranking; and the resolution ladder clears it on homogeneous traffic
  where binary escalation does not (\S\ref{sec:policy}, \S\ref{sec:ladder}).
  \item \textbf{A cost-derived decision rule, and the cascade assumption it
  corrects.} Calibrating the expected signed gain and thresholding at the
...
prices, and the read cost eats the routing gain. The
probe clears the hull everywhere, by most at tight budgets, and
\S\ref{sec:confound} shows its ranking is largely a domain effect, so the
margin is owed to the mid-prefill abort rather than to what the direction
knows. And on the homogeneous pilot the ladder clears the hull where binary
...
pages is not waiting on resolution for any of the four models, and the one
explanation that survives is the one about the paper, not the network.
...
wrote, and more useful. No single token budget explains the ceiling, and pixel
information alone is spent by $768$\,px; in the band between $768$ and
$1152$\,px the two are entangled and this paper cannot separate their
contributions. The serving consequence is untouched, because it never depended
on the mechanism: on this corpus every model we tested stops paying by
$1152$\,px, and Algorithm~3 finds that point without needing to know why it is
there.
...
(Algorithm~3). Two of its steps exist because we got them wrong first. Step~2
measures the visual tokens each pixel target actually produces instead of
trusting the target: four of this paper's errors came from a configuration name
that did not determine a cost, and on our own pilot two nominally different
rungs turned out to be the same configuration for $56\%$ of examples. Step~6
refuses to declare a ceiling from a wide interval, which is the failure we
walked into at $n=244$ on an earlier pass and again, mid-collection, on the
out-of-lineage run.
...
transfers. The mechanism behind the ceiling is also only partly settled: we can
rule out a fixed token budget and we can show pixel information alone is spent
by $768$\,px, but the band between $768$ and $1152$\,px moves both quantities
at once in every model whose tokeniser is resolution-sensitive, and separating
their contributions there would need a model whose token budget can be dialled
independently of its input size.
...
any stops holding; it currently checks sixty-three, including the negative
